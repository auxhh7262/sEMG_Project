...
App({
  ...
  onLaunch() {
    ...
    // 【方案D】启动时先BLE获取最新IP再TCP连接，缓存IP仅作BLE失败时的备用
    async function _connectWithLatestIP() {
      // 优先BLE扫描获取最新IP
      try {
        console.log('[App] BLE扫描获取最新IP...')
        await bleClient.scanAndConnect('sEMG_', 1)
        console.log('[App] BLE连接成功，等待IP回调...')
        // IP会通过 onIpReceived 回调自动触发TCP连接，这里无需额外操作
      } catch (err) {
        console.log('[App] BLE失败，尝试缓存IP:', err.message || err)
        // BLE失败时降级用缓存IP
    ...
    bleClient.onIpReceived((ip, port, deviceData) => {
      ...
      if (needReconnect) {
        console.log('[App] IP变更或TCP断开，强制使用新IP重连:', ip)
