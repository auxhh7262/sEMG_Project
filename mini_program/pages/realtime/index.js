...
const MAX_HISTORY = 5; // 显示5条数据
const HARD_LIMIT = MAX_HISTORY * 2; // 硬上限防止内存溢出
...
Page({
  ...
  _stopStream() {
    ...
    wifiClient.sendQuery('stop', {}).catch(() => {});
  },
  ...
  async _resumeStream() {
    ...
    try {
      wifiClient.sendQuery('start_stream', {}).then((data) => {
        log('[realtime] start_stream ACK received, fw_time:', data.fw_time);
        this._fwStartTime = data.fw_time ? data.fw_time * 1000 : null;
        getApp().globalData.fwTime = data.fw_time || null;
      }).catch(e => {
      ...
      wifiClient.sendQuery('query_cz', {})
        .then(status => {
          if (status?.has_curve === true || status?.has_curve === 1) {
            ...
            // 无拟合曲线时，检查是否有校准数据（通过 get_calib_result 获取）
            const user = storage.getCurrentUser();
            return wifiClient.sendQuery('get_calib_result', { slot: user ? user.slot : 0 });
          }
        })
        .then(calibRes => {
