...
Page({
  ...
  _silentConnect() {
    ...
    if (wifiClient.isConnecting()) return;
  ...
  async _queryAlgorithmAndLoad() {
    try {
      ...
      if (res && (res.has_curve === true || res.has_curve === 1)) {
        this.setData({ hasCurve: true, algorithm: '个性化曲线' });
      } else {
        // 无拟合曲线时，检查是否有校准数据
        const calibRes = await wifiClient.sendQuery('get_calib_result', { slot: user ? user.slot : 0 }).catch(() => null);
        if (calibRes && calibRes.ok && calibRes.rest_rms > 0) {
      ...
      log('[analysis] _queryAlgorithmAndLoad failed:', e);
  ...
  async _fetchAllCzData(startTs, endTs) {
    let allPoints = [];
    // query_cz: 第一批数据通过sendQuery返回，后续批次(more=true)通过push通道推送
    return new Promise((resolve) => {
      ...
      const pushHandler = (data) => {
        ...
        batchCount++;
        // 收到最后一批(no more) 或超安全限制
        if (!data.more || allPoints.length >= SAFETY_LIMIT) {
          wifiClient.offPush(pushHandler);
          if (!resolved) { resolved = true; resolve(allPoints.slice(0, SAFETY_LIMIT)); }
        }
      };
      ...
      wifiClient.sendCmd('query_cz', { start_ts: startTs, end_ts: endTs }).then(firstRes => {
        ...
        if (firstBatch.length > 0) {
          allPoints = allPoints.concat(firstBatch);
        }
        batchCount++;
        // 如果第一批就是最后一批，直接resolve
        if (!firstRes.more || allPoints.length >= SAFETY_LIMIT) {
          wifiClient.offPush(pushHandler);
          if (!resolved) { resolved = true; resolve(allPoints.slice(0, SAFETY_LIMIT)); }
        }
      }).catch(() => {
        wifiClient.offPush(pushHandler);
        if (!resolved) { resolved = true; resolve(allPoints); }
      });
  ...
  _drawChart() {
    ...
    const rawPts = this.data._chartPoints;
    ...
    let pts = rawPts;
    if (rawPts.length > MAX_DRAW_POINTS) {
      const step = Math.ceil(rawPts.length / MAX_DRAW_POINTS);
      pts = rawPts.filter((_, i) => i % step === 0 || i === rawPts.length - 1);
    }
