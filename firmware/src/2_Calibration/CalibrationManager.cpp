...
void CalibrationManager::_endRestPhase() {
    ...
    _restValidCount = count;  // [FIX] 记录有效样本数
...
void CalibrationManager::_endMaxPhase() {
    ...
    _maxValidCount = usedCount;  // [FIX] 记录有效样本数
...
// [FIX-slot] 注入指定slot的校准数据（用于get_calib_result读非当前slot）
void CalibrationManager::setCalibData(const CalibData_t* data) {
    if (!data) return;
    memcpy(&_data, data, sizeof(CalibData_t));
    // 注入的数据视为有效，设置足够大的样本数以通过validateResult()检查
    _restValidCount = 100;
    _maxValidCount = 100;
}

bool CalibrationManager::validateResult() {
    // [FIX] 最小样本数约束：避免样本不足时误判
    if (_restValidCount < 20 || _maxValidCount < 20) {
        LOG("[CALIB] FAIL: insufficient samples (REST=%d, MAX=%d, need >=20)\n", _restValidCount, _maxValidCount);
        return false;
    }
