...
static uint8_t g_activeSlot = 0;  // 当前活跃用户slot（0或1），save_user/load_user/save_calib时更新
...
// [FIX-slot] 按slot读取指定扇区的校准数据（不改变g_currentCalibData）
bool StorageManager::GetPersonalCalibBySlot(uint8_t slot, PersonalCalibData_t* data) {
    if (!data || slot > 1) return false;
    uint32_t sectorAddr = (slot == 0) ? ZONE_A_BASE_ADDR : (ZONE_A_BASE_ADDR + SECTOR_SIZE);
    // 读原始扇区到临时缓冲（不覆盖g_workBuf）
    uint8_t tmp[sizeof(AZone_Sector_t)];
    flash.ReadBytes(sectorAddr, tmp, sizeof(AZone_Sector_t));
    AZone_Sector_t* sec = (AZone_Sector_t*)tmp;
    if (sec->magic != 0xA55AA55A) {
        LOG("[STORAGE] GetPersonalCalibBySlot: slot %d empty\n", slot);
        return false;
    }
    // 映射到PersonalCalibData_t
    data->rest_rms_mv = sec->rest_rms_mv;
    data->max_rms_mv  = sec->max_rms_mv;
    data->rest_mdf_hz = sec->rest_mdf_hz;
    data->max_mdf_hz  = sec->max_mdf_hz;
    data->calib_timestamp = sec->calib_timestamp;
    data->calib_quality   = sec->calib_quality;
    data->peak_rms_mv     = sec->peak_rms_mv;
    memcpy(data->curve_coef, sec->curve_coef, sizeof(data->curve_coef));
    data->curve_baseline_rms = sec->curve_baseline_rms;
    data->curve_baseline_mdf = sec->curve_baseline_mdf;
    data->matched_bcurve_id   = sec->matched_bcurve_id;
    data->fit_confidence      = sec->fit_confidence;
    data->curve_version       = sec->curve_version;
    data->has_curve           = sec->has_curve;
    data->curve_status        = sec->curve_status;
    LOG("[STORAGE] GetPersonalCalibBySlot: slot=%d loaded\n", slot);
    return true;
}

uint8_t StorageManager::GetActiveSlot() const { return g_activeSlot; }

// [S0-2-fix] UpdatePersonalCalib: 完整A区双缓冲写入 + CRC16校验
bool StorageManager::UpdatePersonalCalib(const PersonalCalibData_t* data, uint8_t slot) {
    if (!data) return false;
    if (slot == 0xFF) slot = g_activeSlot;  // 默认用当前活跃用户
    if (slot > 1) {
        LOG("[STORAGE] UpdatePersonalCalib: invalid slot %d\n", slot);
        return false;
    }
    g_activeSlot = slot;  // 更新当前活跃用户

    // [slot-fix] 直接写指定 slot 的扇区（不再用双缓冲交替写）
    // 策略：先读当前扇区内容到 g_workBuf，更新校准字段，再写回
    uint32_t sectorAddr = _getAZoneSectorAddr(slot);
    flash.ReadBytes(sectorAddr, &g_workBuf, sizeof(AZone_Sector_t));
    ...
    g_asyncAZoneTargetSector = slot;
    g_asyncAZonePending = true;

    g_asyncEraseState = ASYNC_ERASE_AZONE_SECTOR;
    g_asyncEraseAddr = _getAZoneSectorAddr(slot);
    ...
    LOG("[STORAGE] Calib data: async erase sector %d (slot=%d) started (CRC=0x%04X)\n",
        slot, slot, g_workBuf.header_crc16);
    return true;
}
...
bool StorageManager::CZone_AppendDataPoint(const CZone_DataPoint_t* dataPoint) {
    ...
    if (g_czone_block_write_offset == sizeof(CZone_BlockHeader_t) + sizeof(CZone_DataPoint_t)) {
        ...
        LOG("[STORAGE] CZone: set block start_ts=%lu\n", (unsigned long)ntpTs);
    }
...
void StorageManager::tick() {
    ...
    case ASYNC_ERASE_CZONE_BLOCK: {
        ...
        hdr.start_timestamp = 0;  // [FIX] Will be set to NTP time when first data point arrives
        hdr.num_points = 0;
...
bool StorageManager::CZone_QueryByTimeRange(uint32_t startTs, uint32_t endTs,
    ...
    // 快速检查：读第一个有效块的header，如果magic不对说明C区无数据
    CZone_BlockHeader_t firstHdr;
    LOG("[CZQ] reading first header at 0x%08lX\n", (unsigned long)C_ZONE_BASE_ADDR);
    flash.ReadBytes(C_ZONE_BASE_ADDR, &firstHdr, sizeof(firstHdr));
    LOG("[CZQ] first header magic=0x%08lX status=%d\n", (unsigned long)firstHdr.magic, firstHdr.block_status);
    if (firstHdr.magic != 0x435A424C || firstHdr.block_status == 0) {
        *outCount = 0;
        *outNextTs = 0;
        return false;
    }
    ...
    *outNextTs = 0;
    ...
    for (uint32_t blockAddr = C_ZONE_BASE_ADDR, _scanCount = 0; 
        ...
        for (uint16_t i = 0; i < numPoints && *outCount < maxPoints; i++) {
            ...
            // timestamp_ms实际存储的是秒级NTP时间戳（getCurrentTimestamp返回秒）
            uint32_t ptTsSec = pt.timestamp_ms;
            
            // [DEBUG] 第一个数据点的时间戳
            if (i == 0 && blockAddr == g_czone_current_block_addr) {
                LOG("[CZQ] first pt: ts_ms=%lu ptTsSec=%lu startTs=%lu endTs=%lu\n",
                    (unsigned long)pt.timestamp_ms, (unsigned long)ptTsSec,
...
bool StorageManager::GeneratePersonalCurve(uint8_t bcurve_id,
    ...
    bool ok = UpdatePersonalCalib(&g_currentCalibData, g_activeSlot);
    if (ok) {
